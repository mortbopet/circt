...
void addSOSTAttributes(OperationState &result, int size, Type dataType,
...
// Parses a Sized Operation with Single Type (SOST). These are operation on the
// format:
...
ParseSOSTOperation(OpAsmParser &parser,
                   ...
                   bool explicitSize, bool alwaysControl = false) {
  if (explicitSize)
  ...
  addSOSTAttributes(result, size, type, alwaysControl);
  return success();
}

static void printSOSTOp(OpAsmPrinter &p, Operation *op, bool explicitSize) {
  if (explicitSize) {
...
void ForkOp::build(OpBuilder &builder, OperationState &result, Value operand,
  ...
  addSOSTAttributes(result, outputs, type);
}
...
static ParseResult parseForkOp(OpAsmParser &parser, OperationState &result) {
  ...
  int size;
  if (ParseSOSTOperation(parser, allOperands, result, size, type,
                         /*explicitSize=*/true))
    return failure();
...
void LazyForkOp::build(OpBuilder &builder, OperationState &result,
  ...
  bool isControl = ((dyn_cast<ControlMergeOp>(op) || dyn_cast<StartOp>(op)) &&
                    operand == op->getResult(0))
                       ? true
                       : false;
  addSOSTAttributes(result, outputs, type, isControl);
}
...
void MergeOp::build(OpBuilder &builder, OperationState &result,
  ...
  addSOSTAttributes(result, operands.size(), type);
}
...
void printMergeOp(OpAsmPrinter &p, MergeOp op) { printSOSTOp(p, op, false); }
...
void MuxOp::build(OpBuilder &builder, OperationState &result, Value operand,
  ...
  for (int i = 0, e = inputs; i < e; ++i)
    result.addOperands(operand);
  addSOSTAttributes(result, inputs, type);
}
...
static ParseResult parseMuxOp(OpAsmParser &parser, OperationState &result) {
  ...
  addSOSTAttributes(result, size, dataType);
...
void handshake::BranchOp::build(OpBuilder &builder, OperationState &result,
  ...
  bool isControl = ((dyn_cast<ControlMergeOp>(op) || dyn_cast<StartOp>(op)) &&
                    dataOperand == op->getResult(0))
                       ? true
                       : false;
  addSOSTAttributes(result, 1, type, isControl);
}
...
void SinkOp::build(OpBuilder &builder, OperationState &result, Value operand) {
  result.addOperands(operand);
  addSOSTAttributes(result, 1, operand.getType());
}
...
void handshake::BufferOp::build(OpBuilder &builder, OperationState &result,
                                ...
                                bool sequential) {
  result.addOperands(operand);
  addSOSTAttributes(result, size, innerType);
...
void printJoinOp(OpAsmPrinter &p, JoinOp op) { printSOSTOp(p, op, false); }
