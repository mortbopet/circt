...
// CHECK:           %[[VAL_5:.*]]:2 = fork(%[[VAL_1]]#3) {control = true} : (none) -> (none, none)
// CHECK:           %[[VAL_6:.*]]:2 = fork(%[[VAL_1]]#2) {control = true} : (none) -> (none, none)
...
// CHECK:           %[[VAL_35:.*]] = join(%[[VAL_34]]#1, %[[VAL_5]]#1, %[[VAL_6]]#1, %[[VAL_1]]#4) {control = true} : (none, none, none, none) -> none
...
// CHECK:           %[[VAL_39:.*]]:3 = fork(%[[VAL_38]]) : (index) -> (index, index, index)
// CHECK:           %[[VAL_40:.*]], %[[VAL_3]] = load(%[[VAL_39]]#2, %[[VAL_1]]#0, %[[VAL_33]]#2) : (index, f32, none) -> (f32, index)
// CHECK:           %[[VAL_41:.*]]:2 = fork(%[[VAL_40]]) : (f32) -> (f32, f32)
// CHECK:           %[[VAL_42:.*]] = arith.addf %[[VAL_41]]#0, %[[VAL_41]]#1 : f32
// CHECK:           %[[VAL_43:.*]] = join(%[[VAL_33]]#1, %[[VAL_5]]#0) {control = true} : (none, none) -> none
// CHECK:           %[[VAL_2]]:2 = store(%[[VAL_42]], %[[VAL_39]]#1, %[[VAL_43]]) : (f32, index, none) -> (f32, index)
// CHECK:           %[[VAL_44:.*]] = join(%[[VAL_33]]#0, %[[VAL_6]]#0) {control = true} : (none, none) -> none
// CHECK:           %[[VAL_45:.*]], %[[VAL_4]] = load(%[[VAL_37]]#1, %[[VAL_1]]#1, %[[VAL_44]]) : (index, f32, none) -> (f32, index)
// CHECK:           sink(%[[VAL_45]]) : (f32) -> ()
// CHECK:           %[[VAL_15]] = branch(%[[VAL_31]]) : (index) -> index
// CHECK:           %[[VAL_18]] = branch(%[[VAL_35]]) {control = true} : (none) -> none
// CHECK:           %[[VAL_20]] = branch(%[[VAL_39]]#0) : (index) -> index
// CHECK:           %[[VAL_46:.*]]:2 = control_merge(%[[VAL_27]]) {control = true} : (none) -> (none, index)
