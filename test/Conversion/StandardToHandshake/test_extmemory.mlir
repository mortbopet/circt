...
// CHECK:           %[[VAL_4:.*]]:2 = fork(%[[VAL_1]]) {control = true} : (none) -> (none, none)
// CHECK:           %[[VAL_5:.*]]:2 = fork(%[[VAL_4]]#1) {control = true} : (none) -> (none, none)
// CHECK:           %[[VAL_6:.*]] = join(%[[VAL_5]]#1, %[[VAL_2]]#1) {control = true} : (none, none) -> none
// CHECK:           %[[VAL_7:.*]] = constant(%[[VAL_5]]#0) {value = 0 : index} : (none) -> index
// CHECK:           %[[VAL_8:.*]], %[[VAL_3]] = load(%[[VAL_7]], %[[VAL_2]]#0, %[[VAL_4]]#0) : (index, i32, none) -> (i32, index)
